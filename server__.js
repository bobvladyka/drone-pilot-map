...
if (ref) {
  ...
  `INSERT INTO consents (
    user_id, consent_type, consent_text, ip_address, user_agent, granted
...
if (req.body.public_contact === 'on') {
  await pool.query(
  `INSERT INTO consents (user_id, consent_type, consent_text, ip_address, user_agent, timestamp, granted)
   VALUES ($1, 'public_contact', $2, $3, $4, NOW(), TRUE)
...
const consentRes = await pool.query(
  `SELECT 1 FROM consents 
   WHERE user_id = $1 AND consent_type = 'public_contact' AND granted = TRUE 
   LIMIT 1`,
  [row.id]
);
...
// gdpr_registration
const gdprRes = await pool.query(
  `SELECT 1 FROM consents 
   WHERE user_id = $1 AND consent_type = 'gdpr_registration' AND granted = TRUE 
   LIMIT 1`,
  [row.id]
);
row.hasGdprConsent = gdprRes.rowCount > 0;

// bezpečnostní pojistky – bez consentu neservíruj kontakty
if (!row.hasPublicConsent || !row.hasGdprConsent) {
  row.email = null;
  row.phone = null;
}
